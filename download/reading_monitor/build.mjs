...
function conversionCard(e) {
  const chips = ['近3月有實際領課', 'Arkio 廣告組歸因'];
  ...
  <article class="card a conversion-card" data-tier="purchase">
    <div class="chead"><span class="rank">#${e.rank}</span><span class="tag tc">💳 有訂閱</span></div>
    ...
    <div class="cbody">
      ...
      <div class="mrow">
        <div class="m"><b>${e.purchases}</b><span>Arkio訂閱</span></div>
        <div class="m"><b>${e.arkioLeads.toLocaleString()}</b><span>Arkio累計領課</span></div>
        <div class="m"><b>${e.conversionRate != null ? e.conversionRate + '%' : '—'}</b><span>領課→訂閱</span></div>
      </div>
...
const conversionFrom = conversionData[0]?.metaWindowFrom || '—';
const conversionTo = conversionData[0]?.metaWindowTo || '—';
const conversionSection = `
  <section class="acc conversion-acc" id="acc-conversion" data-acc="conversion" style="--ac:#7c3aed">
    <div class="acc-head">
      <div class="acc-title"><h2>近3個月有訂閱的有效廣告</h2><p>${conversionFrom}～${conversionTo} 有實際領課，且 Arkio 投放調控「訂閱」大於 0</p></div>
      <div class="acc-kpis">
        <div class="k"><b>${convertedAdsets}</b><span>有訂閱廣告組</span></div>
        <div class="k"><b>${conversionData.length}</b><span>獨立代表視覺</span></div>
        <div class="k"><b>${conversionPurchases}</b><span>Arkio訂閱累計</span></div>
        <div class="k"><b>${conversionArkioLeads.toLocaleString()}</b><span>Arkio累計領課</span></div>
        <div class="k"><b>${conversionRate != null ? conversionRate.toFixed(2) + '%' : '—'}</b><span>領課→訂閱率</span></div>
      </div>
    </div>
    <div class="conversion-note"><b>近3月口徑：</b>${conversionFrom}～${conversionTo} 期間 Meta 至少有 1 次實際領課，且你指定的 Arkio「Meta 投放調控」頁面中，該廣告組的「訂閱」大於 0；依訂閱數排序，同數時以領課→訂閱率較高者優先。<b>Arkio 的訂閱欄位目前是廣告組累計值，頁面沒有日期切片</b>，因此日期範圍用來限定近期仍有實際領課的廣告，不把累計值誤寫成「近3月新增訂閱」。多素材廣告組以近3月 Meta 領課較多的素材作代表圖，並標示共同歸因；視覺已跨模塊去重。${conversionUpdatedAt ? ` Arkio 更新時間：${esc(conversionUpdatedAt)}。` : ''}</div>
...
const fullNav = `<a href="#acc-conversion" style="--ac:#7c3aed">近3月有訂閱 <b>${conversionData.length}</b></a><a href="#acc-math" style="--ac:#c14f30">台灣數學 Top <b>${mathData.length}</b></a>${nav}`;
...
<title>廣告創意監控台 · 有訂閱 × 閱讀 × 台灣數學 ｜ Rose Rose</title>
<style>
...
<div class="wrap">
  <header class="hero">
    <h1>📊 廣告創意監控台 · 有訂閱 × 閱讀 × 台灣數學</h1>
    <p>資料截至 ${genDate}。新增「近3個月有訂閱」有效廣告模塊；閱讀帳號各以 22 個實際有領課廣告為目標，台灣數學榜依近兩個月領課排名。同圖跨城市、複本、投放主與模塊皆做視覺去重。</p>
    <div class="kpis">
      <div class="kpi"><b>${conversionPurchases}</b><span>Arkio 訂閱累計</span></div>
      <div class="kpi"><b>${conversionData.length}</b><span>近3月有效代表視覺</span></div>
  ...
  <div class="method">
    <b>口徑：</b>領課＝Meta 像素 initiate_checkout；訂閱＝Arkio「Meta 投放調控」的 H5 <code>subs_total</code>。Meta 成效資料源為 Arkio 代理 Meta Insights（廣告層級）。
    閱讀榜單依近4月領課由高至低排序，領課相同時以 CTR 較高者優先；同一底圖跨城市、複本或投放主以感知雜湊去重並合併成效。
    <b>✅ 近4月有領課</b>＝期間內至少有1次可歸屬領課；不再使用素材庫或無成效廣告補數。
    <br>訂閱模塊區間：<b>${conversionFrom} → ${conversionTo}</b>；閱讀區間：<b>${readingFrom} → ${readingTo}</b>，每個帳號目標 22 個；客觀不足時僅列出真實有效數量。訂閱模塊優先保留已轉化視覺，其餘閱讀與數學區會排除相同視覺。另有<b>台灣數學廣告 Top ${mathData.length}</b>，按近2月領課由高至低排名並標示投放主、CTR、CPL與領課；不足 20 張時不以重複或零領課素材補數。
  </div>
...
${sections}
  ...
  <footer>
    Rose Rose ｜ 廣告優化 · 廣告創意監控台（有訂閱 × 投放主 × Top創意）<br>
    圖片點擊放大；CPL＝區間花費÷領課；訂閱榜依 Arkio 訂閱累計排序，閱讀榜依近4月領課排序，數學榜依近2月領課排序。<br>
    產出時間：${genDate}
  </footer>
</div>
</body>
